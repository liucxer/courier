...
func (g *OperationGenerator) WriteOperation(ctx context.Context, method string, path string, operation *oas.Operation) {
	...
	ctxWithMeta := `ctx = ` + g.File.Use("github.com/liucxer/courier/metax", "ContextWith") + `(ctx, "operationID","` + g.ID(id) + `")`

	g.File.WriteBlock(
		codegen.Func(
			codegen.Var(codegen.Type(g.File.Use("context", "Context")), "ctx"),
			codegen.Var(codegen.Type(g.File.Use("github.com/liucxer/courier/courier", "Client")), "c"),
			codegen.Var(codegen.Ellipsis(codegen.Type(g.File.Use("github.com/liucxer/courier/courier", "Metadata"))), "metas"),
		).
			Return(
				codegen.Var(codegen.Type(g.File.Use("github.com/liucxer/courier/courier", "Result"))),
			).
			Named("Do").
		...
		g.File.WriteBlock(
			codegen.Func(
				...
				Return(
					...
					codegen.Var(codegen.Type(g.File.Use("github.com/liucxer/courier/courier", "Metadata"))),
