...
func line(box Bound, in LineString, open bool) MultiLineString {
	if len(in) == 0 {
		return nil
	}
...
func push(out MultiLineString, i int, p Point) MultiLineString {
	if i >= len(out) {
...
func ring(box Bound, in LineString) LineString {
	var out LineString
	if len(in) == 0 {
		return in
	}
...
func bitCode(b Bound, p Point) int {
	code := 0
	if p[0] < b.Min[0] {
		code |= 1
	} else if p[0] > b.Max[0] {
...
func bitCodeOpen(b Bound, p Point) int {
	code := 0
	if p[0] <= b.Min[0] {
		code |= 1
	} else if p[0] >= b.Max[0] {
...
func intersect(box Bound, edge int, a, b Point) Point {
	if edge&8 != 0 {
		// top
		return Point{a[0] + (b[0]-a[0])*(box.Max[1]-a[1])/(b[1]-a[1]), box.Max[1]}
	} else if edge&4 != 0 {
		// bottom
		return Point{a[0] + (b[0]-a[0])*(box.Min[1]-a[1])/(b[1]-a[1]), box.Min[1]}
	} else if edge&2 != 0 {
		// right
		return Point{box.Max[0], a[1] + (b[1]-a[1])*(box.Max[0]-a[0])/(b[0]-a[0])}
	} else if edge&1 != 0 {
		// left
		return Point{box.Min[0], a[1] + (b[1]-a[1])*(box.Min[0]-a[0])/(b[0]-a[0])}
	}
	panic("no edge??")
}
