...
func (p *GrowingPacker) growNode(width, height int) *Node {
	...
	shouldGrowRight := canGrowRight && (p.root.Height >= (p.root.Width + width)) // attempt to keep square-ish by growing right when height is much greater than width
	shouldGrowDown := canGrowDown && (p.root.Width >= (p.root.Height + height))  // attempt to keep square-ish by growing down  when width  is much greater than height
