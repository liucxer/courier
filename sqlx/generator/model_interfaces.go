...
func (m *Model) WriteTableName(file *codegen.File) {
	file.WriteBlock(
		codegen.DeclVar(codegen.Var(codegen.Star(
			codegen.Type(file.Use("github.com/liucxer/courier/sqlx/builder", "Table"))),
			m.VarTable(),
		)),
...
func (m *Model) WriteTableInterfaces(file *codegen.File) {
	...
	m.Columns.Range(func(col *builder.Column, idx int) {
		...
		file.WriteBlock(
			codegen.Func().
				...
				Return(codegen.Var(codegen.Star(codegen.Type(file.Use("github.com/liucxer/courier/sqlx/builder", "Column"))))).
				Do(
	...
	file.WriteBlock(
		codegen.Func(
			...
			MethodOf(codegen.Var(m.PtrType(), "m")).
			Return(codegen.Var(codegen.Type(file.Use("github.com/liucxer/courier/sqlx/builder", "SqlCondition")))).
			Do(
				codegen.Expr(`table := db.T(m)`),
				codegen.Expr(`fieldValues := `+file.Use("github.com/liucxer/courier/sqlx/builder", "FieldValuesFromStructByNonZero")+`(m)

conditions := make([]`+file.Use("github.com/liucxer/courier/sqlx/builder", "SqlCondition")+`, 0)
...
condition := `+file.Use("github.com/liucxer/courier/sqlx/builder", "And")+`(conditions...)
`),

				func() codegen.Snippet {
					if m.HasDeletedAt {
						return codegen.Expr(
							`condition = `+file.Use("github.com/liucxer/courier/sqlx/builder", "And")+`(condition, table.F(?).Eq(0))`,
	...
	if len(m.Keys.Indexes) > 0 {
		...
		file.WriteBlock(
			codegen.Func().
				Named("Indexes").
				MethodOf(codegen.Var(m.Type())).
				Return(codegen.Var(codegen.Type(file.Use("github.com/liucxer/courier/sqlx/builder", "Indexes")))).
				Do(
