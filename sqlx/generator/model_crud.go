...
func (m *Model) WriteCreate(file *codegen.File) {
	file.WriteBlock(
		codegen.Func(codegen.Var(
			codegen.Type(file.Use("github.com/liucxer/courier/sqlx", "DBExecutor")), "db")).
			Named("Create").
...
return err
`,
					codegen.Id(file.Use("github.com/liucxer/courier/sqlx", "InsertToDB")),
				),
			),
	)
	...
	if len(m.Keys.UniqueIndexes) > 0 {
		...
		file.WriteBlock(
			codegen.Func(
				codegen.Var(codegen.Type(file.Use("github.com/liucxer/courier/sqlx", "DBExecutor")), "db"),
...
`),
					...
					codegen.Expr(`
fieldValues := `+file.Use("github.com/liucxer/courier/sqlx/builder", "FieldValuesFromStructByNonZero")+`(m, updateFields...)
`),
...
additions := `+file.Use("github.com/liucxer/courier/sqlx/builder", "Additions")+`{}

switch db.Dialect().DriverName() {
case "mysql":	
	additions = append(additions, `+file.Use("github.com/liucxer/courier/sqlx/builder", "OnDuplicateKeyUpdate")+`(table.AssignmentsByFieldValues(fieldValues)...))
case "postgres":
	...
	additions = append(additions,
			`+file.Use("github.com/liucxer/courier/sqlx/builder", "OnConflict")+`(indexFields).
				DoUpdateSet(table.AssignmentsByFieldValues(fieldValues)...))
}

additions = append(additions, `+file.Use("github.com/liucxer/courier/sqlx/builder", "Comment")+`("User.CreateOnDuplicateWithUpdateFields"))

expr := `+file.Use("github.com/liucxer/courier/sqlx/builder", "Insert")+`().Into(table, additions...).Values(cols, vals...)
	...
	file.WriteBlock(
		codegen.Func(codegen.Var(
			...
			Do(
				codegen.Expr(`
_, err := db.ExecExpr(
`+file.Use("github.com/liucxer/courier/sqlx/builder", "Delete")+`().
From(
	db.T(m),
`+file.Use("github.com/liucxer/courier/sqlx/builder", "Where")+`(m.ConditionByStruct(db)),
`+file.Use("github.com/liucxer/courier/sqlx/builder", "Comment")+`(?),
),
)
...
`+file.Use("github.com/liucxer/courier/sqlx/builder", "Select")+`(nil).
From(
	db.T(m),
`+file.Use("github.com/liucxer/courier/sqlx/builder", "Where")+`(`+toExactlyConditionFrom(file, fieldNames...)+`),
`+file.Use("github.com/liucxer/courier/sqlx/builder", "Comment")+`(?),
),
m,
)
`,
				...
				file.WriteBlock(
					codegen.Func(
						codegen.Var(codegen.Type(file.Use("github.com/liucxer/courier/sqlx", "DBExecutor")), "db"),
						codegen.Var(codegen.Type(file.Use("github.com/liucxer/courier/sqlx/builder", "FieldValues")), "fieldValues"),
					).
...
result, err := db.ExecExpr(
	`+file.Use("github.com/liucxer/courier/sqlx/builder", "Update")+`(db.T(m)).
		Where(
...
`,
				...
				file.WriteBlock(
					codegen.Func(
						...
						Do(
							codegen.Expr(`
fieldValues := ` + file.Use("github.com/liucxer/courier/sqlx/builder", "FieldValuesFromStructByNonZero") + `(m, zeroFields...)
...
From(
	db.T(m),
`+file.Use("github.com/liucxer/courier/sqlx/builder", "Where")+`(`+toExactlyConditionFrom(file, fieldNames...)+`),
`+file.Use("github.com/liucxer/courier/sqlx/builder", "ForUpdate")+`(),
`+file.Use("github.com/liucxer/courier/sqlx/builder", "Comment")+`(?),
),
m,
)
...
`+file.Use("github.com/liucxer/courier/sqlx/builder", "Delete")+`().
	From(db.T(m),
	`+file.Use("github.com/liucxer/courier/sqlx/builder", "Where")+`(`+toExactlyConditionFrom(file, fieldNames...)+`),
	`+file.Use("github.com/liucxer/courier/sqlx/builder", "Comment")+`(`+string(file.Val(m.StructName+"."+methodForDelete).Bytes())+`),
))
`,
...
fieldValues := `+file.Use("github.com/liucxer/courier/sqlx/builder", "FieldValues")+`{}`),
...
_, err := db.ExecExpr(
	`+file.Use("github.com/liucxer/courier/sqlx/builder", "Update")+`(db.T(m)).
		Where(
			`+toExactlyConditionFrom(file, fieldNames...)+`,
			`+file.Use("github.com/liucxer/courier/sqlx/builder", "Comment")+`(`+string(file.Val(m.StructName+"."+methodForSoftDelete).Bytes())+`),
		).
		Set(table.AssignmentsByFieldValues(fieldValues)...),
)
...
func toExactlyConditionFrom(file *codegen.File, fieldNames ...string) string {
	buf := &bytes.Buffer{}
	buf.WriteString(file.Use("github.com/liucxer/courier/sqlx/builder", "And"))
	buf.WriteString(`(
`)
