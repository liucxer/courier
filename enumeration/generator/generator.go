...
import (
	...
	"github.com/liucxer/courier/enumeration/scanner"
...
func (e *Enum) WriteTypeNameAndConstValues(file *codegen.File) {
	...
	tpe := codegen.Slice(codegen.Type(file.Use("github.com/liucxer/courier/enumeration", "IntStringerEnum")))
...
if o, ok := (interface{})(v).(?); ok {
	offset = o.Offset()
}`, codegen.Id(file.Use("github.com/liucxer/courier/enumeration", "DriverValueOffset")))
...
*v = ?(i)
return nil`,
				codegen.Id(file.Use("github.com/liucxer/courier/enumeration", "ScanIntEnumStringer")),
				codegen.Id(e.Name),
			),
		),
	)
